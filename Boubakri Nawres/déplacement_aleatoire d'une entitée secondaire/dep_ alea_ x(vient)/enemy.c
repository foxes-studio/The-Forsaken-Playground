...
void initialiser_enemy(enemy* E)
{
    ...
    E->image_enemy = /*IMG_Load("fishright.jpeg");//*/SDL_LoadBMP("fish.bmp");
...
int enemy_vient(enemy E,SDL_Surface*screen)
{
SDL_Event event;
    ...
    while (continuer)
    {
        ...
        if (tempsActuel - tempsPrecedent > 30) /* Si 30 ms se sont écoulées depuis le dernier tour de boucle */
        {
               ...
               if(k==0)
                     ...
                     { 
                            if(E.position_enemy.x>pos)//E.positionmin_enemy.x)//pos)
                          {
                            E.position_enemy.x--;
